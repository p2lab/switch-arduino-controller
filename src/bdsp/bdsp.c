...
/* Static functions */
static void soft_reset_seq(void);
...
/*
 * Soft reset sequence from in-battle
 */
static void soft_reset_seq(void)
{
	set_leds(BOTH_LEDS);
	...
	// Press X to prompt Close game dialog
	send_update(BT_X, DP_NEUTRAL, S_NEUTRAL, S_NEUTRAL);
	send_update(BT_NONE, DP_NEUTRAL, S_NEUTRAL, S_NEUTRAL);
	// Press A to close game
	send_update(BT_A, DP_NEUTRAL, S_NEUTRAL, S_NEUTRAL);
	send_update(BT_NONE, DP_NEUTRAL, S_NEUTRAL, S_NEUTRAL);
	// Press A to open game
	send_update(BT_A, DP_NEUTRAL, S_NEUTRAL, S_NEUTRAL);
	send_update(BT_NONE, DP_NEUTRAL, S_NEUTRAL, S_NEUTRAL);
	// Wait X seconds till we get to title screen
	_delay_ms(1400);
	// Press A to init game
	send_update(BT_A, DP_NEUTRAL, S_NEUTRAL, S_NEUTRAL);
	send_update(BT_NONE, DP_NEUTRAL, S_NEUTRAL, S_NEUTRAL);
	// Move forward to encounter Palkia
	send_update(BT_NONE, DP_TOP, S_NEUTRAL, S_NEUTRAL);
	_delay_ms(500);
	send_update(BT_NONE, DP_NEUTRAL, S_NEUTRAL, S_NEUTRAL);
	// Press A to close dialog and start battle
	send_update(BT_A, DP_NEUTRAL, S_NEUTRAL, S_NEUTRAL);
	send_update(BT_NONE, DP_NEUTRAL, S_NEUTRAL, S_NEUTRAL);
